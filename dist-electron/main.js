...
const loginRateLimiter = new security_1.RateLimiter(5, 15); // 5 attempts per 15 minutes
const registerRateLimiter = new security_1.RateLimiter(3, 60); // 3 attempts per hour
...
electron_1.app.whenReady().then(() => {
    ...
    autoSaveManager = new autosave_1.AutoSaveManager(db, {
        intervalMs: 5000, // Auto-save every 5 seconds
        enabled: true,
    });
    autoSaveManager.start();
    (0, autosave_1.registerAutoSaveHandlers)(autoSaveManager);
    // Initialize PDF export service
    pdfExportService = new pdfExport_1.PDFExportService(db, documentsPath);
...
electron_1.ipcMain.handle('auth:register', async (event, { email, password, firstName, lastName }) => {
    try {
        ...
        const sanitizedEmail = security_1.InputSanitizer.sanitizeEmail(email);
        if (!security_1.InputSanitizer.isValidEmail(sanitizedEmail)) {
        ...
        const passwordValidation = (0, security_1.validatePasswordStrength)(password, [
            sanitizedEmail,
            firstName,
            lastName,
        ].filter(Boolean));
        ...
        const id = generateId();
        const passwordHash = await bcrypt_1.default.hash(password, 12); // Increased rounds for better security
        const stmt = db.prepare(`
      INSERT INTO users (id, email, password_hash, first_name, last_name)
      VALUES (?, ?, ?, ?, ?)
    `);
        stmt.run(id, sanitizedEmail, passwordHash, firstName || null, lastName || null);
        const token = jsonwebtoken_1.default.sign({ userId: id, email: sanitizedEmail }, JWT_SECRET, { expiresIn: '30d' });
...
electron_1.ipcMain.handle('auth:validate-password', async (event, { password, userInputs = [] }) => {
    try {
        const validation = (0, security_1.validatePasswordStrength)(password, userInputs);
        return {
            success: true,
            validation,
        };
...
// ============================================================
electron_1.ipcMain.handle('export:with-annotations', async (event, { documentId, outputPath }) => {
    try {
...
electron_1.ipcMain.handle('export:annotations-summary', async (event, { documentId, outputPath }) => {
    try {
...
electron_1.ipcMain.handle('export:select-output-path', async () => {
